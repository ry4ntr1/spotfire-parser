...
def process_dxp(dxp_path, output_dir):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Write CSVs for DataTables
        for dt in im.get("DataTables", []):
            safe_name = re.sub(r"[^A-Za-z0-9_-]+", "_", dt.get("Name", "")).strip("_")
            csv_path = os.path.join(output_dir, f"{base_name}_{safe_name}.csv")
            with open(csv_path, "w", newline="", encoding="utf-8") as csvfp:
                writer = csv.writer(csvfp)
                writer.writerow(["Column", "DataType", "Expression"])
